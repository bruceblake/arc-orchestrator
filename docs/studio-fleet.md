...
| Gameplay QA needs a human: models do not play like players | human playtest sign-off at milestones: Studio → Playtest runs any build (main or a task branch) with F8 findings and a survey; ACCEPTED findings feed the next `studio plan`; the recorded playtest video is what the human reviews first |
