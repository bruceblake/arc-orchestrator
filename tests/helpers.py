...
def _differently_priced_pair():
    """Two live models whose per-token rates differ AND are both non-zero, or None.
    ...
    """
    live = [m for m in config.IMPLEMENTER_MODELS
            if config._price_for(m) and any(config._price_for(m))]
    for a in live:
        for b in live:
