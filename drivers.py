...
def _reasonix_config_toml(models):
    """The fleet's reasonix.toml: one `arc` provider over the ARC endpoint.
    ...
    arc/<name>` resolves for any of them; the ARC model id is passed through
    verbatim, which is what the gateway expects. The sandbox is off because
    reasonix's shell tool refuses to run at all without bubblewrap — "refusing
    to run unconfined" on this WSL box — and a gate that cannot run tests is
    a gate that fails every task. Permissions are handled by
    --permission-mode on argv; [permissions] mode=allow is the belt to that
    brace so no writer fallback ever waits for an answer nobody gives.
    """
    quoted = ", ".join(json.dumps(m) for m in models)
    return (
        "# Generated by arc-orchestrator (drivers.reasonix_fleet_home); edits\n"
        "# are overwritten whenever ARC_API_KEY, ARC_BASE_URL or the roster change.\n"
        f"default_model = \"arc/{models[0]}\"\n\n"
        "[[providers]]\n"
        "name           = \"arc\"\n"
        "kind           = \"openai\"\n"
        ...
        "web_search     = false\n"
        f"context_window = {config.OPENCODE_CONTEXT}\n\n"
        "[permissions]\n"
        "mode = \"allow\"\n\n"
        "[sandbox]\n"
        "bash = \"off\"\n\n"
...
class ReasonixDriver(Driver):
    ...
    def argv(self, prompt, session_id):
        ...
        # spawn() sets to the worktree.
        a = [config.reasonix_bin(), "run", "--model", f"arc/{self.model}",
